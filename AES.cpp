...
void splitStringToBytes(string key) {
	...
	ofstream keyFile("key.txt", ios::out);
	if (!keyFile) {
	...
	{
		keyFile << (key[i]) << (key[i + 1]) << (key[i + 2]) << " "; // שמירה בבסיס עשרוני

	}
}


// פונקציה ליצירת מפתח רנדומלי ושמירתו בקובץ key.txt בבסיס עשרוני
void generateRandomKey(u8* key, int length) {
	random_device rd;
	mt19937 gen(rd());
	uniform_int_distribution<> dis(0, 255);

	ofstream keyFile("keyToRSA.txt", ios::out);
	if (!keyFile) {
		cerr << "file key.txt cannot be opened\n";
		exit(1);
	}
	...
	for (int i = 0; i < length; ++i) {
		key[i] = dis(gen);
		int num = static_cast<int>(key[i]);
		if (num < 10)
			keyFile << "00" << static_cast<int>(key[i]); // שמירה בבסיס עשרוני
		else if (num < 100)
			keyFile << "0" << static_cast<int>(key[i]); // שמירה בבסיס עשרוני
		else
			keyFile << static_cast<int>(key[i]); // שמירה בבסיס עשרוני
	}
	keyFile.close();
}
...
void Dec() {
	...
	for (int i = 0; i < 16; ++i) {
		int value;
		keyFile >> value;
		cipherKey[i] = static_cast<u8>(value); // קריאה בבסיס עשרוני
	}
	keyFile.close();
